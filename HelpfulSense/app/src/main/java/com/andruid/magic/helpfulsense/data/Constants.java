package com.andruid.magic.helpfulsense.data;

public class Constants {
    public static final int NO_OF_TABS = 3, POS_ALERT = 0, POS_MESSAGE = 1, POS_CONTACTS = 2,
            CONTACTS_PICKER_REQUEST = 0, NOTI_ID = 1;
    public static final String FILE_CONTACTS = "contacts.json", FILE_ACTIONS = "actions.json";
    public static final String ACTION_ADD = "action_add", ACTION_SMS = "action_sms",
            INTENT_LOC_SMS = "intent_loc_sms", INTENT_SERVICE_STOP = "intent_service_stop",
            KEY_MESSAGE = "key_message";
    public static final String CHANNEL_ID = "channel_sensor", CHANNEL_NAME = "Sensor Service";
}